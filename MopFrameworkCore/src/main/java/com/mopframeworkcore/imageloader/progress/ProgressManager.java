 ...
 */
package com.mopframeworkcore.imageloader.progress;
...
import com.mopframeworkcore.imageloader.progress.body.ProgressRequestBody;
import com.mopframeworkcore.imageloader.progress.body.ProgressResponseBody;
...
public final class ProgressManager {
    ...
    public void notifyOnSuccess(String url) {
        checkNotNull(url, "url cannot be null");
        forEachListenersOnSuccess(mRequestListeners, url);
        forEachListenersOnSuccess(mResponseListeners, url);
    }
    ...
    private void forEachListenersOnError(Map<String, List<ProgressListener>> map, String url, Exception e) {
        if (map.containsKey(url)) {
            ...
            for (ProgressListener pl : array) {
                pl.onError(-1, e);
            }
        }
    }

    private void forEachListenersOnSuccess(Map<String, List<ProgressListener>> map, String url) {
        if (map.containsKey(url)) {
            map.remove(url);
        }
    }
